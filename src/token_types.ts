...
export function getMutabilityConfig(config: any, expectedNumber: number): boolean[] {
    if (config === undefined) {
        return new Array(expectedNumber).fill(false);
    } else {
        return (config as boolean[]).length == expectedNumber ? config: new Array(expectedNumber).fill(false);
    }
}
...
export class RoyaltyConfig {
    ...
    constructor(
        creators: [TxnBuilderTypes.AccountAddress, number][],
    ) {
